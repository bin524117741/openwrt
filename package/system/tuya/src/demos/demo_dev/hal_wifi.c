...
#include "../../sdk/include/tuya_gw_wifi_types.h"
...
int hal_wifi_get_cur_channel(uint8_t *channel)
{
    ...
    char *chan_str = NULL;
    FILE *fp = NULL;
    //log_err("hal_wifi_get_cur_channel");
    fp = popen("iwlist "WLAN_DEV" channel | grep \"Current Frequency\" ", "r");
    if (fp == NULL) {
        //log_err("+");
        return -1;
    }
...
int hal_wifi_connect_station(char *ssid, char *passwd)
{
    ...
    snprintf(cmd, sizeof(cmd), "uci set wireless.@wifi-iface[0].ssid=%s", ssid);
    system(cmd);

    snprintf(cmd, sizeof(cmd), "uci set wireless.@wifi-iface[0].key=%s", passwd);
    system(cmd);
    system("uci set wireless.@wifi-iface[0].mode=sta");
    system("uci commit wireless");
    system("wifi down");
    system("wifi up");
    // g_wk_mode = WMODE_STATION;	
