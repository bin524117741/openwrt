...
//sdk版本号
#define TUYA_PROD_SDK_VERSION "1.0.2"
...
#define TUYA_FRAME_MAX_LEN          512
#define TUYA_UG_FRAME_MAX_LEN      (10*1024 + TUYA_FRAME_MIN_LEN)
...
typedef enum
{
    ...
    TUYATEST_RELAY_TEST = 0x43,
    TUYATEST_IR_SEND_TEST = 0x3C,
    TUYATEST_IR_RECV_TEST = 0x3D,
    TUYATEST_IR_MODE_TEST = 0x3E,
    TUYATEST_DLTJ_CAL_TEST = 0x47, // 电量统计校准
    TUYATEST_MAX
} TUYATEST_CMD;
...
typedef struct {    
    ...
    TYTEST_COMMON_HANDL upgrade_start_cb;               //21.upgrade start    
    ...
    TYTEST_COMMON_HANDL_NOBUF relay_test_cb;            //43.relay test callback
    TYTEST_COMMON_HANDL_NOBUF ir_led_test_cb;           //44.ir led test callback
    TYTEST_COMMON_HANDL_NOBUF ir_send_test_cb;          //45.ir send test callback
    TYTEST_COMMON_HANDL_NOBUF ir_recv_test_cb;          //46.ir recv test callback
    TYTEST_COMMON_HANDL_NOBUF ir_mode_test_cb;          //47.ir mode test callback
    TYTEST_COMMON_HANDL_NOBUF dltj_cal_test_cb;         //71.dltj cal test callback
}TUYA_TEST_CBS_S;
...
//other system func cbs
typedef void (*TYTEST_NOLINUX_MSLEEP)(unsigned long timems);
typedef int (*TYTEST_NOLINUX_CREATE_THREAD)(thread_process thr_process,void **handle);
typedef int (*TYTEST_NOLINUX_CANCEL_THREAD)(void *handle);
typedef struct tuya_nolinux_func_handler{
    TYTEST_NOLINUX_MSLEEP msleep;
    TYTEST_NOLINUX_CREATE_THREAD create_thread_and_run;
    TYTEST_NOLINUX_CANCEL_THREAD cancel_thread;
}TNFH_HANDLER;

//uart func cbs
typedef int  (*TYTEST_UART_READ_READY)(void);
typedef unsigned int  (*TYTEST_UART_READ)(unsigned char *data, unsigned int len);
typedef unsigned int (*TYTEST_UART_WRITE)(unsigned char *data, unsigned int len);
typedef struct tuya_uart_func_handler{
    TYTEST_UART_READ_READY read_ready;
    TYTEST_UART_READ uread;
    TYTEST_UART_WRITE uwrite;
}TPUART_HANDLER;
...
int tuya_prodtest_deinit(void);
int tuya_prodtest_register_nolinux_func(TNFH_HANDLER *ctrl);
int tuya_prodtest_register_uart_func(TPUART_HANDLER *ctrl);
