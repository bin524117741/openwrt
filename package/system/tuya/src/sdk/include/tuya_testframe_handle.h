...
typedef int (*TYTEST_F_W_CFG_HANDL)(char *auzkey, char *uuid, char *pid, int prodtest, char *ap_, char *ap_pwd,char *psk);
typedef int (*TYTEST_F_R_CFG_HANDL)(char *auzkey, char *uuid, char *pid, int *prodtest, char *ap_ssid, char *ap_pwd,char *psk);
...
typedef void (*TYTEST_F_IR_LED_HANDL)(TYPT_IR_LED_STATE state);
typedef int (*TYTEST_F_IR_MODE_HANDL)(TYPT_IR_MODE state);
...
typedef void (*TYTEST_F_RELAY_HANDL)(char *stat);
typedef int (*TYTEST_F_IR_SEND_HANDL)(char *ir_data);
typedef int (*TYTEST_F_IR_RECV_HANDL)(char *ir_data);
typedef int (*TYTEST_F_UG_START_HANDL)(unsigned long *size);
typedef int (*TYTEST_F_SLAVE_FIRM_HANDL)(SLAVE_TYPE type,char *buf);

typedef struct {    
    TYTEST_F_WR_HANDL enter_prodtest_frame_cb;          //1.enter prodtest    
    TYTEST_F_WR_HANDL exit_prodtest_frame_cb;           //1.exit prodtest
    TYTEST_F_WR_HANDL master_firm_frame_cb;                 //2.master_firm
    TYTEST_F_SLAVE_FIRM_HANDL slave_firm_frame_cb;          //3.slave firm
    ...
    TYTEST_F_UG_START_HANDL upgrade_start_frame_cb;           //21.upgrade start    
    ...
    TYTEST_F_RELAY_HANDL relay_frame_cb;                    //37 relay test
    TYTEST_F_IR_LED_HANDL ir_led_frame_cb;                  //38 ir led test
    TYTEST_F_IR_SEND_HANDL ir_send_frame_cb;                //39 
    TYTEST_F_IR_RECV_HANDL ir_recv_frame_cb;                //40 
    TYTEST_F_IR_MODE_HANDL ir_mode_frame_cb;
    TYTEST_F_COMMON_HANDL dltj_cal_frame_cb;                    //71 dltj test 
}TYTEST_FRAME_CBS_S;
...
int tuya_testframe_register_nolinux_func(TNFH_HANDLER *ctrl);
int tuya_testframe_register_uart_func(TPUART_HANDLER *ctrl);
